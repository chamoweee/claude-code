...
| [trading-agent](./trading-agent/) | Paper-trading research agent for stocks, ETFs, forex, and crypto using live market data (Twelve Data / Crypto.com) — never places real orders | **Command:** `/trading-agent` - `init`/`run`/`status` against a simulated portfolio<br>**Skill:** `trading-agent` - Portfolio management, risk rules, reporting<br>**Agent:** `market-analyst` - Per-symbol data gathering and signal generation |
